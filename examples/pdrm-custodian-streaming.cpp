...
main(int argc, char* argv[])
{
  ...
  // Consumer
  ndn::AppHelper consumer("ns3::ndn::PDRMConsumerStreaming");
  consumer.SetAttribute("Lambda", StringValue(c.lambda));
  consumer.SetAttribute("BatchSize", StringValue(c.batch_size));
  ...
  for (uint32_t i = 0; i < c.mobile_producers / 2; i++) {
    ...
    ndn::LinkControlHelper::FailLink(nodes.Get(0), nodes.Get(c.routers+(c.mobile_producers / 2)+i));
  }
