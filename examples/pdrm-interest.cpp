...
struct Configuration {
 ...
 string homeagent_prefix;
 string register_prefix;
 string unregister_prefix;
};
...
parseInput(string inputfile)
{
 ...
 infile >> parameter >> c.homeagent_prefix;
 infile >> parameter >> c.register_prefix;
 infile >> parameter >> c.unregister_prefix;
...
main(int argc, char* argv[])
{
  ...
  // Mobile Producer
  ndn::AppHelper mobileProvider("ns3::ndn::PDRMInterest");
  ...
  mobileProvider.SetAttribute("HomeAgentPrefix", StringValue(c.homeagent_prefix)); 
  mobileProvider.SetAttribute("RegisterPrefix", StringValue(c.register_prefix)); 
  mobileProvider.SetAttribute("UnregisterPrefix", StringValue(c.unregister_prefix)); 

  for (uint32_t i = 0; i < c.mobile_producers; i++)
    mobileProvider.Install(nodes.Get(c.routers+i));

  // Home Agent
  ndn::AppHelper homeAgent("ns3::ndn::PDRMHomeAgent");
